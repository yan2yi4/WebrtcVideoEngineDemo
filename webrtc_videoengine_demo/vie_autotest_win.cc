...
//________________
mod_ViEAutoTestWindowManager::mod_ViEAutoTestWindowManager()
	: _window1(NULL),
	_terminate(false),
	_eventThread(*webrtc::ThreadWrapper::CreateThread(
	EventProcess, this, webrtc::kNormalPriority,
...
void* mod_ViEAutoTestWindowManager::GetWindow1() {
	return _window1;
}

int mod_ViEAutoTestWindowManager::CreateOneWindow(AutoTestRect window1Size,
	void* window1Title) {
	_hwnd1Size.Copy(window1Size);
	//_hwnd2Size.Copy(window2Size);
	memcpy(_hwnd1Title, window1Title, TITLE_LENGTH);
...
int mod_ViEAutoTestWindowManager::TerminateWindows() {
	_eventThread.SetNotAlive();
...
bool mod_ViEAutoTestWindowManager::EventProcess(void* obj) {
	return static_cast<mod_ViEAutoTestWindowManager*> (obj)->EventLoop();
}

bool mod_ViEAutoTestWindowManager::EventLoop() {
	_crit.Enter();
...
int mod_ViEAutoTestWindowManager::ViECreateWindow(HWND &hwndMain, int xPos,
	int yPos, int width, int height,
	TCHAR* className) {
	HINSTANCE hinst = GetModuleHandle(0);
	WNDCLASSEX wcx;
...
int mod_ViEAutoTestWindowManager::ViEDestroyWindow(HWND& hwnd) {
	::DestroyWindow(hwnd);
	return 0;
}
