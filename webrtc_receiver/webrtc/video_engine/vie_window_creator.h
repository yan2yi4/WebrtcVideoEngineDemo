...
class mod_ViEAutoTestWindowManagerInterface;
...
class mod_ViEWindowCreator {
public:
	mod_ViEWindowCreator();
	virtual ~mod_ViEWindowCreator();

	// The pointer returned here will still be owned by this object.
	// Only use it to retrieve the created windows.
	mod_ViEAutoTestWindowManagerInterface* CreateOneWindow(char *windowName);

	// Terminates windows opened by CreateTwoWindows, which must
	// have been called before this method.
	void TerminateWindows();
private:
	mod_ViEAutoTestWindowManagerInterface* window_manager_;
};
